...
# origin code is at: https://github.com/unwitting/imageme
...
def _create_index_file(
    ...
    html = [
        ...
        '            .image {',
        '                max-width: 100%; border-radius: 0.3em;',
        '            }',
        '            .selected {',
        '                filter: grayscale(1);',
        '            }',
    ...
    for i_index, image_file in enumerate(image_files):
        ...
        html += [
            '    <td>',
            '    <a href="javascript:process(\'#img' + str(i_index) + '\', \''+link_target+'\')">',
            '        <img id="img' + str(i_index) + '" class="image" src="' + img_src + '">',
            '    </a>',
            '    </td>'
        ]
    ...
    html += [
        ...
        '        function process(pId, pName) {',
        '            $.get("process?" + pName, function(result) {',
        '                if(result["pName"] == pName) {',
        '                    if(result["isMove"]) {',
...
def _get_server_port():
    ...
    """
    return int(sys.argv[3]) if len(sys.argv) >= 4 else 8000
...
def _get_src_from_image(img, fallback_image_file):
    ...
    try:
        ...
        return 'data:image/%s;base64,%s' % (target_format.lower(), b64.decode('utf-8'))
    except IOError as exptn:
...
class ServerHandler(SimpleHTTPRequestHandler):
    def do_GET(self):
        mpath, pName = request.splitquery(self.path)
        if pName is not None and mpath[-8:] == '/process':
            self.do_action(pName)
        else:
    ...
    def do_action(self, p_name):
        s_image = os.path.join(sys.argv[1], p_name)
        t_image = os.path.join(sys.argv[2], p_name)
        if os.path.exists(s_image):
            try:
                shutil.move(s_image, t_image)
                is_move = True
            except Exception as e:
                print(e)
                is_move = False
        elif os.path.exists(t_image):
            try:
                shutil.move(t_image, s_image)
                is_move = False
            except Exception as e:
                print(e)
                is_move = True
            ...
            is_move = True
        result = {'pName': p_name, 'isMove': is_move}
        self.output_json(json.dumps(result))
    # 数据返回到前台

    def output_json(self, content):
...
if __name__ == '__main__':
    ...
    if argc != 4:
        print("USAGE:")
        print("\t(python) python imageme.py <image_dir> <output_dir> <port>")
        print("\t(shell)  imageme <image_dir> <output_dir> <port>")
        exit(-1)
